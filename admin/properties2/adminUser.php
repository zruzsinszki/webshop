<?php
    ...
    class User {
        ...
        public function registrate($username, $email, $password1, $password2) {
            ...
            $msg .= ($this->statement->isAdminUsernameUsed($username)) ? " <li>A választott felhasználónév foglalt!</li> " : "";
            ...
            $msg .= ($this->statement->isAdminEmailUsed($email)) ? " <li>A megadott e-mail címmel már regisztráltak!</li> " : "";
            ...
            if($msg == "") {
                $msg .= (!$this->statement->recordAdminNewUser($username, hash('sha512', $password1), $email)) ? " <li>A regisztráció sikertelen! (Belső hiba)</li> " : "";
            }

            $msg = ($msg == "") ? 
            " <h5 class='text-success'>Sikeres Regisztráció!</h5> <a class='mb-3 d-block' href='adminLogin.php'>Tovább a bejelentkezésre!</a>"
            : "<ul>".$msg."</ul>";
        ...
        public function login($username, $password) {
            if($this->statement->loginAdminDataCheck($username, hash("sha512", $password))) {
                session_start();
                $_SESSION['login'] = true;
                $_SESSION['userId'] = $this->statement->getAdminUserId($username);
            } else {
        ...
        public function loginCheck() {
            session_start();
            $userData = $this->statement->getAdminPublicUserData($_SESSION['userId']);
            if(!empty($userData)) {
